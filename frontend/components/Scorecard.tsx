    ...
    <div className="card p-4 text-sm">
      ...
      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">
        {Object.entries(ev.scores).map(([k, v]) => (
          <div key={k} className="flex items-center gap-2">
            <span className="w-28 shrink-0 text-violet-200 text-xs">{DIMENSION_LABELS[k] ?? k}</span>
            <div className="flex-1 h-2 rounded bg-white/15 overflow-hidden">
              <div className={`h-full ${barColor(v)}`} style={{ width: `${v * 10}%` }} />
            </div>
            <span className="w-5 text-right text-xs tabular-nums text-violet-100">{v}</span>
          </div>
        ))}
      </div>
          ...
          <span
            ...
            className={`px-2 py-0.5 rounded-full text-xs border ${
              hit
                ...
                : "border-white/15 text-violet-300 bg-white/10"
            }`}
      ...
      {ev.missed_concepts?.length > 0 && (
        <p className="mt-3 text-violet-100">
          <span className="text-violet-300">Missed: </span>
